...
- []()
- [Getting Started]()
- VersioningVersion: 1.21 - 1.21.1On this page
...
Maven, the system used for dependency hosting, uses a versioning system that differs from semver in some details (though the general `major.minor.patch` pattern remains the same). The related [Maven Versioning Range (MVR)]() system is used in some places in NeoForge (see [above]()). When choosing your versioning scheme, you should make sure it is compatible with MVR, as otherwise, mods will not be able to depend on specific versions of your mod![PreviousStructuring Your Mod]()[NextRegistries]()
