...
public class SetWeatherFunctionTool implements AIFunctionTool {
    ...
    public FunctionCallResult execute(ToolContext ctx, JsonObject arguments) {
        WeatherParams params = parseWeatherArguments(arguments);
        if (params.error != null) {
            return params.error;
        }
        return setWeather(ctx.server(), params.weather, params.duration);
    }
    ...
    private WeatherParams parseWeatherArguments(JsonObject arguments) {
        ...
        if (weatherResult.hasError()) {
            return new WeatherParams(null, 0, weatherResult.error());
        }
