...
| Method | Description |  || `build` | Creates the `ModConfigSpec`. |  || `configure` | Creates a pair of the class holding the config values and the `ModConfigSpec`. |  |
note
...
| Method | Description |  || `comment` | Provides a description of what the config value does. Can provide multiple strings for a multiline comment. |  || `translation` | Provides a translation key for the name of the config value. |  || `worldRestart` | The world must be restarted before the config value can be changed. |  |
...
#### Additional Config Value Types[​](#additional-config-value-types)
...
## Registering a Configuration[​](#registering-a-configuration)
